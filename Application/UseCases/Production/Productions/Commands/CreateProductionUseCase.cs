...
public class CreateProductionUseCase
{
    ...
    public async Task<ProductionCreatedResponseDto> ExecuteAsync(CreateProductionDto dto)
    {
        ...
        // Obtener repositorios genéricos
        ...
        try
        {
            ...
            var plant = await plantRepo.GetByIdAsync(dto.PlantId);
            if (plant == null)
            {
            ...
            var recipes = await recipeRepo.GetAsync(
                filter: r => r.ProductId == dto.ProductId,
            ...
            await productionRepo.AddAsync(production);
            await _unitOfWork.SaveChangesAsync();
            _logger.LogInformation("✅ Producción creada con ID: {ProductionId}", production.Id);
